...
module.exports = {
	...
	up: async (queryInterface, Sequelize) => {
		...
		const findAllCities = () => City.findAll({
			where: {
				is_deleted: false,
			},
		});
		...
		const findAllTransports = () => Transport.findAll({
			where: {
				is_deleted: false,
			},
		});
		const [tours, cities, categories, transports] = await Promise.all([findAllTour(), findAllCities(), findAllCategories(), findAllTransports()]);
		for (let index = 0; index < tours.length; index += 1) {
			if (Math.random() >= 0.3 && tours[index].status === constants.TOUR_PUBLISHED) {
				data.push({
		...
		return queryInterface.bulkInsert('tours_edits', data, {});
	},
	// eslint-disable-next-line no-unused-vars
	down: (queryInterface, Sequelize) => queryInterface.bulkDelete('tours_edits', null, {}),
};
